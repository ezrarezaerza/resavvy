...
import { Moon, Plus, Sun, AudioLines, MoreHorizontal, Trash2, Settings, LogOut, Compass, PanelLeftOpen, PanelLeftClose } from "lucide-react";
...
export function Sidebar({ 
  ...
  return (
    <aside className={`flex flex-col fixed inset-y-0 left-0 z-[60] md:z-30 bg-white/80 dark:bg-[#0f172a]/80 backdrop-blur-xl border-r border-gray-200 dark:border-white/5 transition-all duration-300 ease-in-out md:relative md:translate-x-0 shrink-0 h-full md:pb-24 ${isCollapsed ? 'md:w-20' : 'md:w-64'} w-64 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="flex-1 overflow-y-auto no-scrollbar flex flex-col gap-4 p-4 pt-20 md:p-4">
        <div className="hidden md:flex flex-col gap-1">
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
            <span className={`text-sm font-medium ${isCollapsed ? 'hidden' : 'block'}`}>Home</span>
          </div>
          <div 
            className={`w-full flex items-center gap-3 p-2 rounded-lg transition-colors cursor-pointer ${
          ...
          >
            ...
            <span className={`text-sm font-medium ${isCollapsed ? 'hidden' : 'block'}`}>Discover</span>
          </div>
          {user && (
            <>
              <div 
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-bar-chart-2 shrink-0"><line x1="18" x2="18" y1="20" y2="10"/><line x1="12" x2="12" y1="20" y2="4"/><line x1="6" x2="6" y1="20" y2="14"/></svg>
                <span className={`text-sm font-medium ${isCollapsed ? 'hidden' : 'block'}`}>Analytics</span>
              </div>
              <div 
                className={`w-full flex items-center gap-3 p-2 rounded-lg transition-colors cursor-pointer ${
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0"><path d="m16 6 4 14"/><path d="M12 6v14"/><path d="M8 8v12"/><path d="M4 4v16"/></svg>
                <span className={`text-sm font-medium ${isCollapsed ? 'hidden' : 'block'}`}>My Library</span>
              </div>
            </>
          )}
        </div>
        ...
        {user && (
          <h3 className={`text-xs text-gray-500 font-semibold mb-3 ${isCollapsed ? 'hidden' : 'block'}`}>
            YOUR PLAYLISTS
          </h3>
        )}
        ...
        <div className="space-y-0.5">
          {user && (
            ...
            >
              <div className={`flex items-center gap-3 min-w-0 overflow-hidden ${isCollapsed ? 'justify-center w-full' : ''}`}>
                ...
                <span className={`text-sm truncate ${isCollapsed ? 'hidden' : 'block'}`}>Liked Songs</span>
              </div>
            </div>
          )}
            ...
            return (
              ...
              >
                <div className={`flex items-center gap-3 min-w-0 overflow-hidden ${isCollapsed ? 'justify-center w-full' : ''}`}>
                  ...
                  <span className={`text-sm truncate ${isCollapsed ? 'hidden' : 'block'}`}>{playlist.name}</span>
                </div>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
      ...
      <div className={`p-4 border-t border-gray-200 dark:border-white/5 mt-auto hidden md:flex ${isCollapsed ? 'justify-center' : ''}`}>
        <button 
        ...
        >
          {isCollapsed ? <Plus size={20} className="shrink-0" /> : <><Plus size={18} className="shrink-0" /> New Playlist</>}
        </button>
      </div>
